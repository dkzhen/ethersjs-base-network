...
async function sendWBTC() {
  ...
  const usdcAddress = process.env.SC_WBTC;
  const usdcAbi = [
  ...
  try {
    ...
    for (const privateKey of privateKeys) {
      ...
      if (balance && balance.gt(ethers.constants.Zero)) {
        ...
        console.log(
          `Wallet ${wallet.address} has a balance of ${balance.toString()} WBTC`
        );
      } else {
        console.log(`Wallet ${wallet.address} has an empty balance of WBTC`);
      }
    }
    ...
    for (const { wallet, balance } of walletsWithBalance) {
      ...
      if (balance && balance.gt(ethers.constants.Zero)) {
        ...
        console.log(`WBTC sent from ${wallet.address} to ${recipientAddress}`);
      }
    }
