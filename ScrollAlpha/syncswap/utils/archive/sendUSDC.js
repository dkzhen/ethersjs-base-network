...
dotenv.config();
async function tokenTransfer(from, to, amount, privateKey) {
  try {
    ...
    const transaction = await tokenContract.transferFrom(from, to, amount, {
      gasLimit: gasLimit,
      gasPrice: gasPrice,
    });
    ...
    console.log(
      `Token USDC transfered successfully. https://blockscout.scroll.io/tx/${transaction.hash}`
    );
  } catch (error) {
    console.error(error);
  }
