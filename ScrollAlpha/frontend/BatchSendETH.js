...
async function transferFee(nameFile, numETHFee) {
  try {
    // Set up the contract interface
    const jsonData = fs.readFileSync(nameFile);
    ...
    const amountETHPerBatch = ethers.utils.parseEther(numETHFee); // Fee amount in ETH
    const overrides = {
