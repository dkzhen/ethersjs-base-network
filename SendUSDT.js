...
async function sendUSDT() {
  ...
  const usdcAddress = process.env.SC_USDT;
  const usdcAbi = [
  ...
  try {
    ...
    for (const privateKey of privateKeys) {
      ...
      if (balance && balance.gt(ethers.constants.Zero)) {
        ...
        console.log(
          `Wallet ${wallet.address} has a balance of ${balance.toString()} USDT`
        );
      } else {
        console.log(`Wallet ${wallet.address} has an empty balance of USDT`);
      }
    }
    ...
    for (const { wallet, balance } of walletsWithBalance) {
      ...
      if (balance && balance.gt(ethers.constants.Zero)) {
        ...
        console.log(`USDT sent from ${wallet.address} to ${recipientAddress}`);
      }
    }
